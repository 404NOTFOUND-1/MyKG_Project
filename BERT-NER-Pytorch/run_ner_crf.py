...
def train(args, train_dataset, model, tokenizer):
    ...
    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)  # 8
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(
        train_dataset)  # local_rank=-1
...
def main():
    ...
    args.output_dir = args.output_dir + '{}-crf'.format(args.model_type)
    if not os.path.exists(args.output_dir):
        os.mkdir(args.output_dir)
