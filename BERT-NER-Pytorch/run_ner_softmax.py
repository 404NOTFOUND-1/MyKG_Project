...
classes = ['O', 'B-PAR', 'I-PAR', 'B-MAT', 'I-MAT', 'B-TYPE', 'I-TYPE']
...
def predict(args, model, tokenizer, prefix=""):
    ...
    # get true labels
    rawtext_path = os.path.join(args.data_dir, "rawtest.json")
    true_label = data_process(rawtext_path)
    y_true = []
    for _, label in true_label:
        y_true.extend(label)
    y_pred = []
    for step, batch in enumerate(test_dataloader):
        ...
        # get predict labels
        y_pred.extend(json_d['tag_seq'].split(' '))
        # get predict labels
        results.append(json_d)
        pbar(step)
    logger.info("\n")
    # plot confusion matrix
    plot_my_confusionMatrix(y_true, y_pred, classes, 'bert_softmax_confusionMatrix.png')
    # plot report
    plot_classification_report(y_true, y_pred, classes, '{}-softmax'.format(args.model_type))
...
def main():
    ...
    args.output_dir = args.output_dir + '{}-softmax'.format(args.model_type)
    if not os.path.exists(args.output_dir):
        os.mkdir(args.output_dir)
